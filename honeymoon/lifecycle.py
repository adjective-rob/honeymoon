"""
HONEYMOON Lifecycle — Pre/post pipeline operations.
...
from honeymoon.task_state import DirtyHiveError
...
def check_repo_clean(repo_path: Path) -> None:
    """Raise DirtyHiveError if the repo has uncommitted changes or is behind remote."""
    ...
    if dirty_files:
        ...
        raise DirtyHiveError("Clean your repository before running HONEYMOON.")
    ...
    try:
        ...
        if behind and behind.isdigit() and int(behind) > 0:
            ...
            raise DirtyHiveError("Local branch is behind remote.")
    except Exception:
        pass
    ...
    if getattr(ctx.config, "automation", None) and getattr(ctx.config.automation, "rebase_before_pr", False):
        ...
        if not ctx.workspace.rebase(auto_abort=False):
            ...
            if ctx.test_command:
                ...
                from honeymoon.agent_runners import run_fix_loop
...
def post_run(
    ...
    if result.get("status") in ("pr_created", "committed", "merged"):
        try:
            from honeymoon.brain_writer import upsert_brain
            from honeymoon.history import extract_patterns_from_messages
...
def write_session_entry(ctx: RunContext, task: Task, result: dict[str, Any]) -> None:
    ...
    # Map HONEYMOON status to Prelude outcome
