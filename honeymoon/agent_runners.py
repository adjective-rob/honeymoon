"""
HONEYMOON Agent Runners — Context builders + agent invocations.
...
from honeymoon.agents import AgentContext, AgentResult
from honeymoon.controller_utils import attest_controller_action
from honeymoon.display import print_plan
from honeymoon.doc_inserter import insert_doc_comments
...
from honeymoon.workspace.tools import ToolViolationError
...
def run_implementer(ctx: RunContext, task: Task, plan: dict) -> AgentResult:
    ...
    brain_hints = ""
    try:
        from honeymoon.brain_writer import read_brain_hints
...
def run_archivist(
    ...
    for pattern in ["*.md", "docs/**/*.md", "doc/**/*.md"]:
        existing_docs.extend(
            ...
            if p.is_file() and ".honeymoon" not in str(p)
        )
