"""
HONEYMOON Workspace Isolation (v2.1)
...
class Workspace:
    ...
    def __init__(self, repo_path: Path, task_id: str, worktree_base: str = ".honeymoon/worktrees") -> None:
        self.repo_path = repo_path.resolve()
        self.task_id = task_id
        self.branch_name = f"honeymoon/{task_id}"
    ...
    def _run_cmd(cmd: list[str], cwd: Path, check: bool = True, capture: bool = False) -> str:
        ...
        if check and result.returncode != 0:
            raise HiveError(f"Git failed: {' '.join(cmd)}\n{result.stderr}")
