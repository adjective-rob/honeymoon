...
|------|---------|---------------|
| `honeymoon/controller.py` | Pipeline orchestration. Runs all agents in sequence. | `run()` (main entry), `_run_pipeline_step()` (dispatcher), `_run_implementer()` (builds implementer context) |
| `honeymoon/agents/implementer.py` | Edit engine. Reads, writes, verifies code. | `build_messages()` (prompt construction), `run()` (tool loop), `IMPLEMENTER_TOOLS` (tool definitions list) |
| `honeymoon/agents/planner.py` | Planning. Produces JSON execution plans. | `build_messages()` (prompt construction), `parse_response()` (JSON validation), `PlanStep`/`ExecutionPlan` (Pydantic schemas) |
| `honeymoon/agents/debugger.py` | Fix engine. Diagnoses and fixes test failures. | `build_messages()`, `run()` (tool loop), `DEBUGGER_TOOLS` (tool definitions list) |
| `honeymoon/router.py` | LLM abstraction via LiteLLM. Budget tracking, retries. | `complete()` (send request), `ContextMonitor.enforce_headroom()`, `BudgetTracker` |
| `honeymoon/context_compressor.py` | Memory management. Shrinks old messages. | `compress_stale_messages()` (content compression), `hard_compact_messages()` (message removal) |
| `honeymoon/prelude.py` | Project context bridge to Prelude CLI. | `compact()`, `query()`, `get_constraints()`, `refresh()` |
| `honeymoon/symbols.py` | AST layer for code navigation via tree-sitter. | `get_function_body()`, `get_class_outline()`, `find_references()`, `invalidate()` |
| `honeymoon/event_bus.py` | Global event system. | `bus.emit(event_type, payload, agent_id)` |
| `honeymoon/runners.py` | Agent runner helpers called by controller. | `run_security()`, `run_release()`, `run_archivist()`, `run_delegated_agent()` |
| `honeymoon/config_loader.py` | YAML config parsing. | `GlitchLabConfig` (Pydantic model for honeymoon.yaml) |
| `honeymoon/workspace/tools.py` | Sandboxed command executor for `run_check`. | `ToolExecutor.execute()` |
...
1. Create agent class in `honeymoon/agents/` extending `BaseAgent`
...
5. Add model mapping in `honeymoon.yaml`

### Writing failure/status records
HONEYMOON uses `.honeymoon/` for runtime state. Files go in `.honeymoon/logs/` (audit.jsonl) or directly in `.honeymoon/` (task_state.json). Use `json.dumps()` + append mode for JSONL files. The directory exists by the time the pipeline runs.
